...
namespace koala
{
    ...
    void KoalaSwitchState::onUpdate(float dt)
    {
        ...
        if (!_sceneView)
        {
            ...
            _hudBG = (ichigo::ActorView *)currentScene->findByName("hudBG")->getView();
    ...
    void KoalaSwitchState::onTouchStarted(ichigo::Agent *agent)
    {
        ...
        else if (_touchID != 0 && director->getTouchList().size() > 1)
        {
            // Mak sure to stop dragging
            director->getTouchPoint(_touchID)->markAsHandled(false);
            _touchID = 0;
            _touchEnding = true;
        }
    }
    ...
    void KoalaSwitchState::onTouchMoved(ichigo::Agent *agent)
    {
        ...
        if (_touchID != 0)
        {
            ...
            if (tp)
            {
                ...
                // Move HUD accordingly
                ichigo::Point hudPos = _hudBG->getPosition();
                hudPos.y += deltaY;
                _hudBG->setPosition(hudPos);
            }
            else
            {
    ...
    void KoalaSwitchState::onTouchEnded(ichigo::Agent *agent)
    {
        ...
        // Send the HUD to correct place
        float leaveHeight = _sceneView->getPosition().y;
        float targetY = CONST_FLOAT("HUDBG_DREAM_Y");
        if (leaveHeight > -(_screenHeight / 2))
        {
            targetY = CONST_FLOAT("HUDBG_REALITY_Y");
        }
        float delta = fabs(leaveHeight - targetY);
        _hudBG->moveTo(ichigo::Point(240, targetY), delta / CONST_FLOAT("HUDBG_ANIM_SPEED"));
    }
}